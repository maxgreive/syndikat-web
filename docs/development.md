...
- `npm run audit:a11y` checks representative local pages at `http://127.0.0.1:4000` against WCAG 2.0 AA and saves the JSON review to `reports/pa11y.json`. The report includes both confirmed Axe violations and findings marked `runnerExtras.needsFurtherReview`, which require manual assessment. Start `npm run dev:jekyll` first. Use `npm run audit:a11y:production` after deployment, or set `PA11Y_BASE_URL` to test a preview, for example `PA11Y_BASE_URL=https://deploy-preview-123--syndikat.netlify.app npm run audit:a11y`.
